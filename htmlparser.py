...
from config import MAX_BLOCKS, MAX_LINKS, MAX_TEXT_CHARS, CHARACTER_MODE
...
class ParsedPage:
    def __init__(self):
        self.title=""; self.blocks=[]; self.links=[]; self.truncated=False
...
class StreamingHTMLParser:
    ...
    def __init__(self):
        self.page=ParsedPage(); self._text=[]; self._tag=[]; self._in_tag=False; self._quote=None
        self._prefix=""; self._href=None; self._in_title=False; self._in_pre=False; self._skip_tag=None
    ...
    def feed(self, data):
        ...
        for ch in data:
            ...
            elif self._skip_tag is None: self._text.append(ch)
    ...
    def _flush_text(self):
        ...
        if self._href and len(self.page.links) < MAX_LINKS:
            number=len(self.page.links)+1; self.page.links.append((self._href,value)); value="[{}] {}".format(number,value)
    ...
    def _handle_tag(self, raw):
        ...
        if not name: return
        if self._skip_tag:
            if closing and name == self._skip_tag: self._skip_tag=None
            return
        if name in ("script","style","noscript"):
            if not closing: self._skip_tag=name
            return
        if closing:
            if name == "title": self._in_title=False
            elif name == "a": self._href=None
        ...
        if name == "title": self._in_title=True
        elif name == "a": self._href=parse_attributes(raw).get("href","")
            ...
            if alt: self._text.append("[Image: {}]".format(alt))
